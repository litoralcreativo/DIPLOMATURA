...
const getSingle = async (id) => {
  try {
    const query =
      "SELECT p.nombre, p.id, c.nombre AS nombreCategoria FROM ?? AS p JOIN ?? AS c ON p.id_categoria = c.id WHERE p.id = ?";
...
const createProductos = async (obj) => {
  const query = "INSERT INTO ?? SET ?";
  const params = [T_PRODUCTOS, obj];
  return await pool.query(query, params);
};

module.exports = { getAll, getSingle, createProductos };
